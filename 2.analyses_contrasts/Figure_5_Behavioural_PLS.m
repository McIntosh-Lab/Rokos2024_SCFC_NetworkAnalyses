...
% Bootstrap Ratio (BSR) values, and plots the bPLS constrasts for Figure 5.

%Before running this script, ensure that you have completed the following:
% 1) Computed the necessary brain metric (i.e., SC_weighted_degree)
...
%      a) `datamat_lst{1}`: Ensure this is set to the brain metric of interest (i.e., SC_weighted_degree).
...
%      a) `lv`: Set this to 1 unless reproducing Figure 5B then set this to 2.
%      b) `ylim`: Adjust the y-axis limits based on the range of the data
%           (e.g., ylim([-1, 1]) for Figure 5B.
...
%       a) Set LV to 1 or adjust LV of interest to 2 for reproducing Figure 5B.
...
% File Names
bPLS_results_file = 'FIGURE5A_bPLS.mat'; % MODIFY file name for saving bPLS results
figure_filename = 'FIGURE5A.png'; % MODIFY file name for saving the contrast figure
bsr_csv_filename = 'FIGURE5A_bsrs.csv'; % MODIFY file name for saving significant BSR values
...
datamat_lst{1} = SC_weighted_degree; % MODIFY
...
lv = 1; % MODIFY: LV of interest. Set lv=1 unless reproducing Figure 5B, then set lv=2.
...
ylim([0, 1]); % Optional: MODIFY to ylim([-1, 1]); for Figure 5B
...
%% Step 4: Extract and Save Significant BSR Values
lv = 1; % MODIFY LV of interest. lv=2 for Figure 5B.
