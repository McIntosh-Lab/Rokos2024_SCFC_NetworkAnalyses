...
% Bootstrap Ratio (BSR) values, and plots the bPLS constrast for Figure 4B.

%Before running this script, ensure that you have completed the following:
% 1) Computed the necessary brain metric (i.e., SC_clustering) 
...
%      a) `datamat_lst{1}`: Esnure this is set to the brain metric of interest (i.e., SC_clustering).
...
%           (e.g., ylim([0, 1]).
%      c) `xticklabels`: Modify this in Step 3 to ensure it reflects the specific behavioural measures you are plotting
%         (e.g., xticklabels = {'Age'; 'Selective Attention'}).

%    Step 4: Extract and Save Significant BSR Values
%       a) Ensure the LV of interest is set to 1.
...
% File Names
bPLS_results_file = 'FIGURE4B_bPLS.mat'; % MODIFY file name for saving bPLS results
figure_filename = 'FIGURE4B.png'; % MODIFY file name for saving the contrast figure
bsr_csv_filename = 'FIGURE4B_bsrs.csv'; % MODIFY file name for saving significant BSR values
...
datamat_lst{1} = SC_clustering; % MODIFY
...
tasks = [2, 3]; % 1 = subject, 2 = age, 3 = selective_attention, 4 = sustained_attention, 5 = executive_attention
...
data = result_behavioural.boot_result.orig_corr(:, lv)*-1;
x = 1:numel(data);
...
ylim([0, 1]); % Optional: MODIFY

% Add error bars
lower = (result_behavioural.boot_result.orig_corr(:, lv) - result_behavioural.boot_result.llcorr(:, lv))*-1;
upper = (result_behavioural.boot_result.ulcorr(:, lv) - result_behavioural.boot_result.orig_corr(:, lv))*-1;
er = errorbar(x, data, upper, lower);
...
% Set X-axis labels
xticklabels = {'Age'; 'Selective Attention'}; % MODIFY: X-axis labels
...
bsr = (result_behavioural.boot_result.compare_u(:, lv))*-1;
